...
collapsed_dev_counts_all <- matrix(data=NA, nrow=nrow(all_count_matrix$abundance),ncol=length(tissue_types))
walker <- 1
...
for (tissue in tissue_types){
  tissue_subset_ids <- meta %>% filter(tissue_type == tissue) %>% pull(id)
  tissue_subset_count <- all_count_matrix$abundance 
  tissue_avg <- rowMeans(tissue_subset_count)
...
ggplot(data=melted_collapsed_dev_all, aes(y=log10(count+1))) + geom_boxplot() +  facet_grid(. ~ tissue_type) + theme_cowplot(12) + theme(
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) + xlab("tissue_types") + geom_hline(yintercept=median(log2(all_counts+1)), colour="red")
