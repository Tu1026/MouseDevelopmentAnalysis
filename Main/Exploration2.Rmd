...
rownames(count_mat) <- oldrownames %>% left_join(pc, by=c(colnames(oldrownames)[1] ="Gene_ID"))
...
```{r fig.height=16, fig.width=16, warning=FALSE}
ggplot(data=test_age, aes(x = -log2(adj.P.Val))) + geom_histogram(binwidth = 0.1)

```
```{r fig.height=16, fig.width=16, warning=FALSE}
for (i in 1:5){
  print(paste0("The ", i ," differentially expressed genes in dev stage 0:  ", sort(test_age$adj.P.Val)[i], "name is:  ", ))
}
