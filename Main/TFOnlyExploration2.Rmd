...
Prepare limma brain vs rest
```{r fig.height=16, fig.width=16, warning=FALSE}
#ggplot(data=test_age, aes(x = -log2(adj.P.Val))) + geom_histogram(binwidth = 0.1) + geom_vline(xintercept = -log(0.05), linetype="dotted", colour="red")
ggplot(data=test_age, aes(x = adj.P.Val)) + geom_histogram(binwidth = 0.01)
dev_stage <- factor(meta$dev_stage)
brain_comp <- c("midbrain", "hindbrain", "forebrain", "neural tube")
organism <- factor(ifelse(meta$tissue_type %in% brain_comp, "Brain", "Non-Brain"))
organism <- relevel(brain, ref="Non-Brain")
design <- model.matrix(~ 0 + organism + dev_stage)
